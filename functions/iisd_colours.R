...
iisd_pals <- list(
  `iisd_main` = iisd_colours("navy_blue", "bright_blue", "orange_red", "bright_orange", "yellow", 
                             "bright_green","mint_green", "deep_teal"))
...
iisd_palettes <- function(palette = "iisd_main", reverse = FALSE, discrete = TRUE, ...) {
  ...
  if (discrete) {
    ...
    stopifnot(palette %in% names(iisd_pals))
    
    function(n) {
      if (n > 8) warning("The IISD Discrete Color Palette only has 8 colors.")
...
scale_colour_iisd <- function(palette = "iisd_main", discrete = TRUE, reverse = FALSE, ...) {
  ...
  if (discrete) {
    discrete_scale("colour", palette, palette = pal, ...)
  } else {
